...
describe('Lottery Contract',() => {
  ...
  it('allows one player can enter', async ()=>{
  ...
  it('requires a min amt to enter', async () =>{
    try{
      await lottery.methods.enter().send({
        from : accounts[0],
        value : 5
  ...
  it('only manager is allowed to call pickWinner', async () =>{
    try{
  ...
  it('Money recieved by winner and resets the players array', async () =>{
    await lottery.methods.enter().send({
      from : accounts[0],
      value : web3.utils.toWei('2','ether')
    });
